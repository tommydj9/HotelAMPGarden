...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    try {
        ...
        $mail->SMTPAuth = true;
        $mail->Username = 'la-tua-email@gmail.com'; // <-- METTI LA TUA
        $mail->Password = 'password-app-specifica'; // <-- METTI LA TUA PASSWORD APP
        $mail->SMTPSecure = PHPMailer::ENCRYPTION_STARTTLS;
        $mail->Port = 587;

        // Mittente e destinatario
        $mail->setFrom($email, "$nome $cognome");
        $mail->addAddress('info@hotelampgarden.it'); // <-- email hotel
